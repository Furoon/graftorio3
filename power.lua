...
--- Resolve the electric network ID attached to one copper side of an entity.
--- Replaces LuaEntity.neighbours.copper, which was removed in Factorio 2.1.
--- @param entity LuaEntity
--- @param connector_id defines.wire_connector_id
--- @return uint? network_id
function copper_side_network(entity, connector_id)
	local connector = entity.get_wire_connector(connector_id, false)
	if not connector or connector.connection_count == 0 then
		return nil
	end
	for _, connection in pairs(connector.connections) do
		local owner = connection.target and connection.target.owner
		if owner and owner.valid then
			return owner.electric_network_id
		end
	end
	return nil
end
...
local function get_ignored_networks_by_switches()
	...
	for switch_id, _ in pairs(script_data.switches) do
		...
		if switch and switch.valid and switch.power_switch_state then
			local left = copper_side_network(switch, defines.wire_connector_id.power_switch_left_copper)
			local right = copper_side_network(switch, defines.wire_connector_id.power_switch_right_copper)
			if left and right then
				ignored[max(left, right)] = true
			end
		end
	end
...
function on_power_destroy(event)
	...
	if entity.type == "electric-pole" then
		...
		-- LuaEntity.neighbours and disconnect_neighbour() no longer exist in 2.x;
		-- copper wires are handled through LuaWireConnector since 2.0.
